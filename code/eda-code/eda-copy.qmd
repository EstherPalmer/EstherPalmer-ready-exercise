...
Boxplot of height stratified by pets owned.
```{r}
p5 <- mydata %>% 
  ggplot(aes(x = Pets_owned, y = Height)) +
  geom_boxplot()

plot(p5)
figure_file <- here("results", "figures", "height-by-pets_owned.png")
ggsave(filename = figure_file, plot = p5)

```

Scatterplot of number of books read as a function of weight.

```{r}
p6 <- mydata %>% 
  ggplot(aes(x = Weight, y = Number_books_read)) +
  geom_point()

plot(p6)
figure_file <- here("results", "figures", "weight-vs-books_read.png")
